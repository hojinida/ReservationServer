...
class PaymentWebhookController(private val paymentService: PaymentService) {
    @PostMapping("/payment")
    fun handlePaymentWebhook(
        @RequestBody request: PaymentWebhookRequest, @RequestHeader("X-Signature") signature: String
    ): ResponseEntity<Unit> {
    ...
    fun handleCancelWebhook(
        @RequestBody request: CancelWebhookRequest, @RequestHeader("X-Signature") signature: String
    ): ResponseEntity<Unit> {
        when (request.status.uppercase()) {
