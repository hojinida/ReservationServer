...
fun PurchaseResult.toResponseEntity(): ResponseEntity<Any> {
    return when (this) {
        ...
        is PurchaseResult.SeatNotExists ->
            ResponseEntity.status(HttpStatus.BAD_REQUEST).body("존재하지 않는 좌석입니다.")
        is PurchaseResult.UserAlreadyReserved ->
            ResponseEntity.status(HttpStatus.CONFLICT).body("이미 예매하신 사용자입니다.")
        is PurchaseResult.SeatAlreadyTaken ->
            ResponseEntity.status(HttpStatus.CONFLICT).body("이미 선점된 좌석입니다.")
        is PurchaseResult.OrderCreationFailed ->
            ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("주문 생성에 실패했습니다: ${message}")
    }
}
